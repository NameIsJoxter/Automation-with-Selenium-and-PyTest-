...
class ProductPage(BasePage):
    ...
    def should_be_message_about_adding(self):
        assert self.is_element_present(*ProductPageLocators.ADDED_PRODUCT_NAME_POPUP), \
            'Product name allert is not presented'

    def should_be_message_basket_total(self):
        assert self.is_element_present(*ProductPageLocators.BASKET_TOTAL_POPUP), \
            'Product name allert is not presented'

    def should_desappear_message_about_adding(self):
        assert self.is_disappeared(*ProductPageLocators.ADDED_PRODUCT_NAME_POPUP), \
            'Success message hasnt desappear, but it should'

    def should_not_be_message_about_adding(self):
        assert self.is_not_element_present(*ProductPageLocators.ADDED_PRODUCT_NAME_POPUP),\
    ...
    def product_name_should_be_present_in_success_message(self):
        ...
        assert product_name.text in product_name_in_message.text, \
            'Wrong product name in message'
    ...
    def product_cost_should_be_equal_basket_total_in_success_message(self):
        ...
        assert product_cost.text in basket_total_in_message.text, \
            'Wrong cost in message'
